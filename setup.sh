...
setup_st_entanglement() {
    ...
    #   - "ST_ENTANGLEMENT_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "ST_ENTANGLEMENT_" variables is documented below.
    #
    ...
    cf_export_bool ST_ENTANGLEMENT_SETUP
    if ${ST_ENTANGLEMENT_SETUP} && ! ${CF_ON_SLURM}; then
        >&2 echo "the ST_entanglement analysis was already succesfully setup"
    ...
    #
    # global variables
    # (ST_ENTANGLEMENT = ST_entanglement, CF = columnflow)
    #

    # start exporting variables
    export ST_ENTANGLEMENT_BASE="${this_dir}"
    export CF_BASE="${cf_base}"
    export CF_REPO_BASE="${ST_ENTANGLEMENT_BASE}"
    export CF_REPO_BASE_ALIAS="ST_ENTANGLEMENT_BASE"
    ...
    if ! ${CF_REMOTE_ENV}; then
        ...
        cf_setup_interactive "${CF_SETUP_NAME}" "${ST_ENTANGLEMENT_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${ST_ENTANGLEMENT_BASE}/bin:${PATH}"
    export PYTHONPATH="${ST_ENTANGLEMENT_BASE}:${ST_ENTANGLEMENT_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${ST_ENTANGLEMENT_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${ST_ENTANGLEMENT_BASE}/modules" ); do
            cf_init_submodule "${ST_ENTANGLEMENT_BASE}" "modules/${m}"
        done
    fi
    ...
    export ST_ENTANGLEMENT_SETUP="true"
}
...
main() {
    ...
    if setup_st_entanglement "$@"; then
        cf_color green "ST_entanglement analysis successfully setup"
        return "0"
    else
        local code="$?"
...
# entry point
if [ "${ST_ENTANGLEMENT_SKIP_SETUP}" != "true" ]; then
    main "$@"
fi
